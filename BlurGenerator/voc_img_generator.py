...
import random
import numpy as np
from Generator.new_coco_generator import img2labelVOC, label2imgVOC, \
    extractPngLabel, extractLabelListREDS, extractLableListVOC, REDS_ROOT, VOC_ROOT
...
def putInObjectsStatic(front_objects, background):

    bg = background.copy()
    bg_h = bg.shape[0]
    bg_w = bg.shape[1]
    for f in front_objects:
        initial_pos = [random.randint(0, bg_h - 1) - f.shape[0] // 2,
                       random.randint(0, bg_w - 1) - f.shape[1] // 2]
        bg, blur_mask = putInObject(f, bg, initial_pos)
    return bg
...
def getFrontImgBasedOnBackground(static_img_paths, front_img_paths, background_path, front_object_limit=None):
    front_objects = []
    static_objects = []
    ...
    for front_img_path in static_img_paths:
        ...
        for m in front_masks:
            ...
            #cv2.imwrite('static_{}.png'.format(count), front_i)
            static_objects.append(front_i)

    bg_img = putInObjectsStatic(static_objects, bg_img)
    count = 0
...
def main():
    bg_test_ids = extractLabelListREDS(REDS_ROOT)[:3500]
    ...
    for bg_id in bg_test_ids:
        ...
        #plt.matshow(flow_map)
        #plt.show()

        cv2.imwrite('/dataset/REDS_AUG/large_sharp_image/{}.png'.format(foldername), frames[gt_index], [int(cv2.IMWRITE_PNG_COMPRESSION), 3])
        cv2.imwrite('/dataset/REDS_AUG/large_blur_image/{}.png'.format(foldername), blur_frame, [int(cv2.IMWRITE_PNG_COMPRESSION), 3])
        np.save('/dataset/REDS_AUG/large_blur_mask/{}.npy'.format(foldername), total_blur_mask)
        img_test = cv2.imread('/dataset/REDS_AUG/large_sharp_image/{}.png'.format(foldername))
        assert (img_test - frames[gt_index]).any() == False
